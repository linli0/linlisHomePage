...
const menuItems = computed(() => {
  ...
  if (features.value.tweets) items.push({ name: '推特', path: '/tweets', icon: '🐦' })
  if (features.value.xiaomi) items.push({ name: '小爱', path: '/xiaomi', icon: '🔊' })
  if (features.value.quant) items.push({ name: '量化', path: '/quant', icon: '📊' })
  if (features.value.ai) items.push({ name: 'AI', path: '/ai', icon: '🤖' })
  return items
})
...
onMounted(async () => {
  ...
  try {
    ...
    if (data) {
      features.value = {
        ai: Boolean(data.ai),
        tweets: Boolean(data.tweets),
        quant: Boolean(data.quant),
        xiaomi: Boolean(data.xiaomi)
      }
    }
  } catch {
    // Keep plugins hidden when features endpoint is unavailable.
  }
})
