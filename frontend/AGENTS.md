...
实现前需做到：
明确列出所有假设；如有不确定，主动提问。
若存在多种解读方式，需全部列出，不可擅自默认选择。
若有更简单的实现方案，主动说明；必要时提出反对意见。
若内容存在模糊不清之处，立即暂停，指出困惑点并提问。
2. 简洁优先
用最少代码解决问题，不编写任何推测性、冗余代码。
不实现需求以外的任何功能。
一次性使用的代码不做抽象封装。
不添加未被要求的 “灵活性” 或 “可配置性”。
不为不可能出现的场景编写异常处理。
若一段功能本可用 50 行实现，却写了 200 行，必须重写。
自问：“资深工程师是否会认为这段代码过于复杂？” 若答案是肯定的，立即简化。
3. 精准修改
仅修改必要部分，只清理自身改动产生的问题。
修改现有代码时：
不 “优化” 相邻代码、注释或格式。
不重构无问题的代码逻辑。
遵循项目现有编码风格，即便与个人习惯不同。
发现无关的无效代码，仅做提示，不擅自删除。
当改动产生冗余内容时：
清理因本次修改而不再使用的导入项、变量、函数。
除非明确要求，否则不删除原本就存在的无效代码。
检验标准：每一行被修改的代码，都应能直接对应到用户需求。
4. 目标导向执行
定义成功标准，反复验证直至达标。
将任务转化为可验证目标：
“添加校验逻辑”→“编写非法输入测试用例，并让用例通过”
“修复缺陷”→“编写可复现问题的测试用例，并让用例通过”
“重构 X 模块”→“确保重构前后所有测试均正常通过”
多步骤任务需简述执行计划：
[步骤] → 验证项：[检查内容]
[步骤] → 验证项：[检查内容]
[步骤] → 验证项：[检查内容]
